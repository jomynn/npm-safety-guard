...
export type TyposquatReason = "typosquat" | "homoglyph" | "non_ascii";

export interface TyposquatHit {
  package: string;
  version: string;
  reason: TyposquatReason;
  closestMatch?: string;
  distance?: number;
  note: string;
}
...
function stripJsSuffix(name: string): string {
  return name
    .replace(/[.-]js$/i, "")
    .replace(/^js[.-]/, "")
    .replace(/[.-]ts$/i, "")
...
export function checkPackageName(name: string, version: string): TyposquatHit | null {
  ...
  if (TOP_PACKAGES.has(name)) return null;
  ...
  if (stripped !== name && TOP_PACKAGES.has(stripped)) return null;
  ...
  for (const top of TOP_PACKAGES) {
    ...
    if (Math.abs(top.length - name.length) > Math.ceil(maxLen * 0.35)) continue;
...
export function checkAllPackageNames(
  ...
  for (const [name, version] of Object.entries(deps)) {
    if (wlSet.has(name.toLowerCase())) continue;

...
function downloadTarball(url: string): Promise<Buffer | null> {
...
// ─── Tar parser (USTAR + GNU long-name + PAX extended header) ────────────────
...
function parseTar(buf: Buffer): TarEntry[] {
  ...
  while (offset + 512 <= buf.length) {
    ...
    if (buf.readUInt8(offset) === 0) break;
...
function snippet(content: string, index: number, len = 80): string {
  const start = Math.max(0, index - 20);
  const end = Math.min(content.length, index + len);
  return content.slice(start, end).replace(/\s+/g, " ").trim();
}

function detect(file: string, content: string): DeepScanFinding[] {
  const found: DeepScanFinding[] = [];
  ...
  type Detector = {
    type: FindingType;
    severity: Severity | ((m: RegExpExecArray) => Severity);
    pattern: RegExp;
    description: string;
    minLength?: number;
  };
  ...
  const BENIGN_PATTERNS: Partial<Record<FindingType, RegExp>> = {
  ...
  for (const det of detectors) {
    ...
    while ((m = re.exec(content)) !== null && count < 5) {
      ...
      let severity: Severity = typeof det.severity === "function" ? det.severity(m) : det.severity;
      if (det.type === "child_process" && isInstallScript) severity = "high";
...
function topSeverity(findings: DeepScanFinding[]): Severity | "none" {
...
function gunzipBuffer(buf: Buffer): Promise<Buffer | null> {
  return new Promise((resolve) => {
    zlib.gunzip(buf, (err, out) => resolve(err ? null : out));
  });
}

const cache = new Map<string, DeepScanResult>();
...
export async function deepScanPackage(
  ...
  for (const entry of entries) {
    ...
    if (!SCANNABLE_EXT.test(display)) continue;
    if (entry.size > MAX_FILE_SCAN_BYTES) continue;
...
export async function deepScanAll(
  deps: Record<string, string>,
  onProgress?: (done: number, total: number, currentPkg: string) => void
  ...
  for (let i = 0; i < entries.length; i += CONCURRENCY) {
    ...
    for (const r of settled) {
      if (r && (r.findings.length > 0 || r.scriptsPresent || r.error)) results.push(r);
    }
  }
...
export function clearDeepScanCache(): void {
  cache.clear();
}

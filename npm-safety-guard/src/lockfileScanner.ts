...
export interface LockfileDep {
  name: string;
  version: string;
  /** Path inside the tree — useful for explaining "where did this come from" */
  path?: string;
}

export interface LockfileSummary {
  format: "npm-v1" | "npm-v2" | "npm-v3" | "yarn-v1" | "unknown";
  totalEntries: number;
  uniqueDeps: LockfileDep[]; // de-duplicated by name@version
}

// ─── npm package-lock.json ────────────────────────────────────────────────────

function parseNpmLockfile(raw: unknown): LockfileSummary | null {
  ...
  if (obj.packages && typeof obj.packages === "object") {
    const format: LockfileSummary["format"] = lfv === 3 ? "npm-v3" : "npm-v2";
    const deps: LockfileDep[] = [];

    for (const [key, meta] of Object.entries<any>(obj.packages)) {
      ...
      if (!extractedName || !meta?.version || typeof meta.version !== "string") continue;
  ...
  if (obj.dependencies && typeof obj.dependencies === "object") {
    const deps: LockfileDep[] = [];
    const walk = (tree: Record<string, any>, prefix = "") => {
      for (const [name, meta] of Object.entries(tree)) {
        ...
        if (meta.dependencies) walk(meta.dependencies, path);
      }
    };
...
function parseYarnLockV1(content: string): LockfileSummary | null {
  // yarn.lock v1 has the comment-header "# yarn lockfile v1".
  if (!/^#\s*yarn lockfile v1/mi.test(content)) return null;
...
function dedupe(deps: LockfileDep[]): LockfileDep[] {
...
export async function parseLockfile(filePath: string): Promise<LockfileSummary | null> {
  let content: string;
  ...
  if (filePath.endsWith("package-lock.json") || filePath.endsWith("npm-shrinkwrap.json")) {
    try { return parseNpmLockfile(JSON.parse(content)); }
    catch { return null; }
  }
